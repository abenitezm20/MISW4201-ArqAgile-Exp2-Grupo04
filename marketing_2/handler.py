...
USUARIO_AUTORIZADOR = { "usuario": "cliente2", "contrasena": "cliente2"}
...
class Handler():
    ...
    def escuchar():
        ...
        if login.status_code != 200:
            logging.error('Error al intentar autenticar componente marketing 2 http code', login.status_code)
            return
        respLogin = json.loads(login.content.decode('utf-8'))
        
        if 'error' in respLogin:
            logging.error('Error al intentar autenticar componente marketing 2 - ' + respLogin['error'])
            return
        else:
